...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><span class="handle">перетянуть</span></div></div>'); // Create drag handle
			...
			// $(obj).append('<span class="lt-arrow" id="lt-arrow'+randID+'"></span><span class="rt-arrow" id="rt-arrow'+randID+'"></span>');

			if(o.showFullLinks)
			{	
				$(obj).after('<div class="balinks" id="links'+randID+'"><span class="balink blink"><a id="showleft'+randID+'" href="javascript:void(0)">'+o.beforeLinkText+'</a></span><span class="balink alink"><a id="showright'+randID+'" href="javascript:void(0)">'+o.afterLinkText+'</a></span></div>');
				$('#links'+randID).width(imgWidth);
				$('#showleft'+randID).click(function(){
					$('div:eq(2)', obj).animate({width:imgWidth},o.linkDisplaySpeed);
					$('#dragwrapper'+randID).animate({left:imgWidth-$('#dragwrapper'+randID).width()-87+'px'},o.linkDisplaySpeed);
				});
				$('#showright'+randID).click(function(){
					$('div:eq(2)', obj).animate({width:0},o.linkDisplaySpeed);
					$('#dragwrapper'+randID).animate({left:'85px'},o.linkDisplaySpeed);
				});
			}
